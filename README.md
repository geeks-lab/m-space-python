...
## 내가 어떤 것들을 했는지
**[추천 파이프라인 설계 및 알고리즘 구현]**
* selenium과 konlpy를 활용하여 데이터 수집 및 가공
* word2vec을 파인튜닝하여 상용모델 Fasttext사용 대비 유사성 3.6배 가량 향상시킴
* KNN알고리즘 사용하여 벡터 간 인접한 벡터를 구하여 서로 다른 식당 사이에 존재하는 식당 구하는 기능 개발

**[백엔드 API 개발(FastAPI-RESTful)]**
* Python으로 백엔드 주요 기능 개발(연관 단어 맵핑 기능, 개인 식당 추천 기능, 교집합 식당 추천 기능)
* FastAPI로 RESTful API 개발
* python 백엔드 서버 Docker로 배포
