...
def restaurants_for_many(restaurant_id_list):
    ...
    if len(two_vectors) == 2:
        rest_c_index = getting_mid_c_indices(two_vectors[0], two_vectors[1]) # rest_c_indices -> 1개
        distances, rest_c_index = knn_model.kneighbors([vector_dataset[rest_c_index]])
        nearest_vector = vector_dataset[int(rest_c_index)]

        for itm in restaurants_dict_from_redis:
            if np.array_equal(nearest_vector, itm['vector']):
                result_res_id =itm['id']
        return result_res_id
    else:
